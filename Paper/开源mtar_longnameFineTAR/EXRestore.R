...
for (file in txt_files) {
  ...
  # 查找所有匹配的压缩比
  matches <- str_extract_all(content, "Restore throughput:\\s*\\d+\\.\\d+")
...
for (i in 1:length(txt_files)) {
  ...
  matches <- regmatches(
    content,
    gregexpr("Restore throughput:\\s*([0-9]+\\.?[0-9]*)", content)
  )
  matches <- unlist(matches)
  
  # 从匹配文本中提取数值，更加健壮的方式
  ratios <- as.numeric(gsub(".*Restore throughput:\\s*([0-9]+\\.?[0-9]*).*", "\\1", matches))
