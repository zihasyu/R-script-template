...
for (i in 1:length(lines)) {
  ...
  # 提取OCR(+Recipe)
  if (grepl("^OCR\\(\\+Recipe\\):", line)) {
    ocr_match <- regmatches(line, regexpr("[0-9]+\\.?[0-9]*$", line))
    if (length(ocr_match) > 0) {
      current_ocr <- as.numeric(ocr_match)
      
      # 当三个值都收集到后，添加到结果中
      if (!is.na(current_dataset) && !is.na(current_b_param) && !is.na(current_ocr)) {
        results <- rbind(results, data.frame(
          Dataset = current_dataset,
          B_Parameter = current_b_param,
          OCR_Recipe = current_ocr,
          stringsAsFactors = FALSE
        ))
      }
    }
...
# 创建数据透视表：数据集为行，B参数为列
pivot_table <- dcast(results, Dataset ~ B_Parameter, value.var = "OCR_Recipe")
...
# 保存工作簿
output_file_full <- "Paper/impact/OCR_Recipe_results.xlsx"
