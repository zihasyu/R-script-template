...
# 读取文本文件
file_path <- "Paper/impact/ImpactchunkInfoLog -r.txt"
...
for (i in 1:length(lines)) {
  ...
  if (grepl("-i /mnt/dataset2/", line)) {
    ...
    # 提取-r参数
    r_match <- regmatches(line, regexpr("-r [0-9]+\\.?[0-9]*", line))
    if (length(r_match) > 0) {
      current_r_param <- as.numeric(sub("-r ", "", r_match))
    }
  }
  ...
  if (grepl("^Overall Compression Ratio:", line)) {
    ...
    if (length(ratio_match) > 0) {
      ...
      # 当三个值都收集到后，添加到结果中
      if (!is.na(current_dataset) && !is.na(current_r_param) && !is.na(current_ratio)) {
        results <- rbind(results, data.frame(
          Dataset = current_dataset,
          R_Parameter = current_r_param,
...
# 创建数据透视表：数据集为行，R参数为列
pivot_table <- dcast(results, Dataset ~ R_Parameter, value.var = "Overall_Compression_Ratio")

# 为列名添加"R_"前缀，使其更清晰
colnames(pivot_table)[-1] <- paste0("R_", colnames(pivot_table)[-1])
...
setColWidths(wb, "Pivot Table", cols = 2:ncol(pivot_table), widths = 15)  # R参数列宽
...
# 保存工作簿
output_file_full <- "Paper/impact/compression_ratio_results_R_parameter.xlsx"
...
cat(paste("- R参数数量:", length(unique(results$R_Parameter)), "\n"))
cat(paste("- R参数值:", paste(sort(unique(results$R_Parameter)), collapse = ", "), "\n"))