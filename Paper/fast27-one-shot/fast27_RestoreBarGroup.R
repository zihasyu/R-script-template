...
# 读取CSV数据
csv_path <- here("Paper/fast27-one-shot/fast27-throughput/restore_throughput.csv")
raw_data <- read.csv(csv_path, stringsAsFactors = FALSE)

# 确保输出文件夹存在
plot_dir <- here("Paper/fast27-one-shot/fast27-throughput/RestoreBar")
...
p <- create_grouped_barplot_precomputed(
  ...
  x_label = "",
  y_label = "Speed (MiB/s)",
  export_name = "fast27_restore_throughput.pdf",
...
cat("已生成分组柱状图: fast27_restore_throughput.pdf\n")
print(p)
