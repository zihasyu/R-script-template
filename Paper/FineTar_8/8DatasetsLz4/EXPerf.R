...
for (file in txt_files) {
  ...
  # 查找所有匹配的压缩比
  matches <- str_extract_all(content, "Throughput:\\s*\\d+\\.\\d+")
...
for (i in 1:length(txt_files)) {
  ...
  matches <- regmatches(
    content,
    gregexpr("Throughput:\\s*([0-9]+\\.?[0-9]*)", content)
  )
  matches <- unlist(matches)
  
  # 从匹配文本中提取数值，更加健壮的方式
  ratios <- as.numeric(gsub(".*Throughput:\\s*([0-9]+\\.?[0-9]*).*", "\\1", matches))
...
# 保存结果到Excel文件
write_xlsx(results, path = file.path(directory, "Throughput.xlsx"))

cat("save to ", file.path(directory, "Throughput.xlsx"), "\n")
