...
# 假设 create_grouped_barplot_with_ci 函数定义在 Bar.R 或当前文件中
# 如果 Bar.R 存在且包含该函数，则取消下一行注释
# source(here("MyR", "Bar.R"))

# --- create_grouped_barplot_with_ci 函数定义 ---
# (为了脚本的完整性，将函数定义包含在此处)
...
# 读取Excel数据
excel_path <- here("Paper/TOS-26-3/MergeRestore.xlsx")
...
# 参数设置
GROUP_SIZE <- 6  # 每组的方法数量
INTERVAL <- 8    # 数据集数量

# 确保输出文件夹存在
plot_dir <- here("Paper/TOS-26-3/RestoreBar")
...
for(i in 1:INTERVAL) {
  ...
  reordered_data <- group_data[order_index]
...
if(length(custom_group_order) != length(all_group_labels) || !all(custom_group_order %in% all_group_labels)) {
...
p <- create_grouped_barplot_with_ci(
  ...
  x_label = "",
  y_label = "Speed (MiB/s)",
  export_name = "grouped_restore_performance_6bars.pdf",
...
cat("已生成分组柱状图（6个柱子版本）: grouped_restore_performance_6bars.pdf\n")
print(p)